...
8. [x] Đổi mật khẩu khi đã đăng nhập + thêm field profile (chốt + xong cả 2 phía 2026-07-20, nhánh backend `feat/user-profile-fields` commit `c5069c0`):
   ...
   - **Đặt ở tab riêng "Quản lý tài khoản"** (đổi 2026-07-20, không nằm chung tab "Hồ sơ" như bản đầu) — `SettingsPage.jsx` giờ có 4 tab: Hồ sơ / **Quản lý tài khoản** / Lịch sử bình luận / Thùng rác. Tab "Quản lý tài khoản" hiện chỉ có `ChangePasswordSection`, sẽ là nơi gom cả tính năng đổi email (bên dưới) khi làm.
   - **Đổi email — HOÃN LẠI, bàn riêng 2026-07-20**: khác đổi mật khẩu, cần OTP xác minh **email mới** (không phải để xác minh danh tính — JWT+password đã lo việc đó — mà để chắc user thực sự sở hữu email mới, tránh bị lợi dụng đổi email rồi tự trigger forgot-password chiếm tài khoản). Thiết kế dự kiến: 2 endpoint riêng `POST /change-email/` (gửi OTP tới email mới) + `POST /verify-email-change/` (xác nhận), y hệt pattern `forgot-password`/`reset-password` đã có — CHƯA làm, cần bàn kỹ hơn trước khi code. Sẽ đặt chung tab "Quản lý tài khoản" với đổi mật khẩu.
