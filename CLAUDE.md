...
- [x] **General Notifications feed (2026-07-23)** — backend shipped a unified `Notification` model (`assigned`/`invited`/`deadline_due` — `commented` was built then removed, see below) covering everything the old invitation-only bell didn't. `src/api/notifications.js` (`listNotifications`/`loadMoreNotifications` — cursor pagination like `activityLogs.js`, no `count`/`?page=` — /`markNotificationRead`/`markAllNotificationsRead`). Display/routing logic shared between the bell and the full page lives in `src/components/notifications/notificationDisplay.js` (`notificationLabel(n)`, `notificationTarget(n)` — `invited` → `/invitations`, others → `/projects/{project_id}/tasks/{task_id}` using `project_id`/`task_id` fields the backend serializer computes at creation time specifically so the frontend doesn't need extra round-trips to resolve them). `Topbar.jsx`'s bell reworked: was invitation-only (`listMyInvitations()`), now pulls from `listNotifications()`, badge counts `is_read===false` in the fetched page (approximate — cursor pagination has no exact unread count), polls every 30s (`POLL_INTERVAL_MS`) since there's no push/websocket. New `NotificationsPage.jsx` at `/notifications` (reached only via the bell's "Xem tất cả", no Sidebar entry — same pattern as `/invitations`) — full list + "Xem thêm" load-more (pattern copied from `ActivityLogFeed.jsx`) + "Đánh dấu tất cả đã đọc" button (uses the Snackbar `useNotification()` from việc #6, not a page-level `Alert`).
  - **Do NOT confuse `src/notifications/NotificationContext.jsx` with this feature** — that's the unrelated Snackbar/toast system (`notifySuccess`/`notifyError`) from Giai đoạn 5 #6, predates this feature, naming collision is coincidental. This feature's own files live under `src/components/notifications/` (note: different directory) and `src/pages/NotificationsPage.jsx`.
  - **`commented` verb removed 2026-07-23** (backend + frontend) after live testing showed 2 real problems: (1) no page felt "live" — bell only fetched once on mount, fixed generally via the polling above; (2) the comment notification's `object_repr` was the comment's own content snippet, with no commenter name, so "Bình luận mới: ..." read as anonymous — rather than fix that (add commenter name into `object_repr`), decided to just cut scope and drop `commented` entirely, keeping only `assigned`/`invited`/`deadline_due`. If this comes back, the label map in `notificationDisplay.js` and the backend signal are the 2 places to touch.

**Giai đoạn 4 — Chờ backend (không code, chỉ note lại)**
- Team chat, Files browser, Account management (2FA/devices/delete), "Task Done over time" chart — no backend support yet.
